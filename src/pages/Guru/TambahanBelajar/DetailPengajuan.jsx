...
const DetailPengajuan = ({ data, onBack, onConfirmAction }) => {
  ...
  // Mengirim pesan ke node "Notifikasi" berdasarkan classId siswa
  const kirimNotifInternal = async (status, alasan = "") => {
    try {
      if (!data.classId) {
        console.warn("ClassId tidak ditemukan, notifikasi mungkin tidak sampai.");
      }
      ...
      const payload = {
        ...
        pesan: status === 'Disetujui' 
          ? `Permintaan tambahan belajar kamu untuk tanggal ${data.tglDiminta} pukul ${data.jamDiminta} telah disetujui oleh tutor.`
          : `Maaf, permintaan tambahan belajar kamu ditolak. Alasan: ${alasan}`,
        createdAt: Date.now(),
        isRead: false,
        type: "tambahan",
        idPengajuan: data.id // Untuk referensi jika siswa mengklik notif
      };

      await push(notifRef, payload);
    } catch (err) {
  ...
  const handleAction = async (id, status, alasan = "") => {
    try {
      // 1. Jalankan aksi utama (biasanya update status di database pengajuan)
      await onConfirmAction(id, status, alasan);
      
      // 2. Kirim Notifikasi ke kelas siswa tersebut
      await kirimNotifInternal(status, alasan);
      
      // 3. Opsional: Berikan feedback ke admin/user yang sedang memakai komponen ini
      alert(`Berhasil mengirim jawaban: ${status}`);
      
      // Tutup form penolakan jika sedang terbuka
  ...
  const styles = {
    ...
    infoBox: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', backgroundColor: isDarkMode ? '#1e293b' : '#f8fafc', padding: '15px', borderRadius: '12px', marginBottom: '20px', border: `1px solid ${colors.border}` },
    actionBtn: (type) => ({
      flex: 1, padding: '14px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontWeight: '700', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px',
  ...
  return (
    <div style={{ width: '100%', animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={styles.card}>
        ...
        {/* ... Alasan Permintaan & Info Penolakan ... */}
        ...
        {data.status === "Menunggu" && !isRejecting && (
          <div style={{ display: 'flex', gap: '12px' }}>
            <button style={styles.actionBtn('approve')} onClick={() => handleAction(data.id, 'Disetujui')}>
              <CheckCircle2 size={18}/> Terima & Beritahu Siswa
            </button>
        ...
        {isRejecting && (
          <div style={{ marginTop: '10px', borderTop: `1px solid ${colors.border}`, paddingTop: '20px' }}>
            ...
            <textarea 
              placeholder="Berikan alasan agar siswa mengerti..." 
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '15px' }}>
              ...
              <button 
                ...
                onClick={() => handleAction(data.id, 'Ditolak', reason)} 
                disabled={!reason}
              >
                Kirim Penolakan
              </button>
