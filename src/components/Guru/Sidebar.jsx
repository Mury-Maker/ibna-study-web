    ...
    <>
      ...
      <aside style={styles.sidebar}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <NavLink path="/Guru/Profile" icon={<User size={20} />} name="Profile" />
          <NavLink path="/Guru/Dashboard" icon={<LayoutDashboard size={20} />} name="Dashboard" />
          <NavLink path="/Guru/Jadwal" icon={<Calendar size={20} />} name="Jadwal" />

          {/* DROPDOWN MANAJEMEN KELAS */}
          <div>
            <div style={styles.menuItem(false)} onClick={() => setIsKelasOpen(!isKelasOpen)}>
              <Users size={20} style={{ marginRight: '12px', color: colors.textMuted }} />
              <span style={{ flex: 1 }}>Manajemen Kelas</span>
              {isKelasOpen ? <ChevronDown size={18} /> : <ChevronRight size={18} />}
            </div>
            
            {isKelasOpen && daftarKelas.map((kelas) => {
              const path = `/Guru/Kelas/${kelas.id}`;
              const isActive = location.pathname.startsWith(path); // Gunakan startsWith agar submenu tetap aktif
              return (
                <div key={kelas.id} style={styles.subMenuItem(isActive)} onClick={() => { navigate(path); if(isMobile) toggleSidebar(); }}>
                  {/* Tambahkan dot kuning kecil jika aktif untuk submenu */}
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: isActive ? colors.secondary : 'transparent', marginRight: '10px' }} />
                  <span>{kelas.nama_kelas} ({kelas.jenjang})</span>
                </div>
              );
            })}
          </div>

          <NavLink path="/Guru/Rekap-Nilai" icon={<FileText size={20} />} name="Rekap Nilai" />
          <NavLink path="/Guru/Tanya-PR" icon={<MessageSquare size={20} />} name="Tanya PR" />
          <NavLink path="/Guru/Tambahan-Belajar" icon={<PlusCircle size={20} />} name="Tambahan Belajar" />
        </div>
